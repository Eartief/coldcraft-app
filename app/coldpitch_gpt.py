                ...
                if st.form_submit_button("Login"):
                    try:
                        ...
                        if not getattr(resp, 'session', None): st.error("Invalid credentials")
                            ...
                            s=resp.session
                            st.session_state.update({"access_token":s.access_token,"refresh_token":s.refresh_token,"authenticated":True,"user_email":resp.user.email,"active_tab":"Generator"})
                            st.rerun()
                    except AuthApiError as e: st.error(f"Login failed: {e}")
                if st.form_submit_button("Continue as Guest"): st.session_state["guest"]=True; st.session_state["active_tab"]="Generator"; st.rerun()
        else:
                    ...
                    if new_pwd!=cnf: st.error("Passwords do not match.")
                        ...
                        try:
                            ...
                            if getattr(resp,'session',None):
                                s=resp.session; st.session_state.update({"access_token":s.access_token,"refresh_token":s.refresh_token,"authenticated":True,"user_email":resp.user.email,"active_tab":"Generator"}); st.success("Account created and logged in!"); st.rerun()
                            else:
    ...
    if "openers" in st.session_state:
        ...
        if st.session_state["authenticated"]:
            if st.button("Save Lead"): 
                try: supabase.table("coldcraft").insert({**st.session_state["generated_lead"],"user_email":st.session_state["user_email"]}).execute(); st.success("Saved!")
        ...
        for lead in leads:
            ...
            with st.expander(txt):
                ...
                if st.button("Delete",key=f"del{lead['id']}"):
                    try: supabase.table("coldcraft").delete().eq("id",lead['id']).execute(); st.success("Deleted"); st.rerun()
