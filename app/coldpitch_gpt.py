    ...
    try:
        ...
        if not leads:
            ...
            for i, lead in enumerate(leads):
                with st.expander(f"{lead.get('lead')[:40]}..."):
                    ...
                    if st.checkbox("Confirm delete", key=confirm_key):
                        if st.button("🗑️ Delete This Lead", key=delete_key):
                            try:
                                supabase.table("coldcraft").delete().eq("timestamp", lead.get("timestamp")).eq("user_email", user_email).execute()
            ...
            if st.checkbox("🧹 Select multiple leads to delete"):
                selections = {f"multi_{i}": st.checkbox(f"Delete: {lead.get('lead')[:40]}", key=f"multi_{i}_cb") for i, lead in enumerate(leads)}
                if st.button("🗑️ Bulk Delete Selected Leads"):
                    try:
                        for i, selected in selections.items():
                            if selected:
                                timestamp = leads[int(i.split('_')[1])]["timestamp"]
                                supabase.table("coldcraft").delete().eq("timestamp", timestamp).eq("user_email", user_email).execute()
...
if "openers" in st.session_state and st.session_state.openers:
    components.html("""
    <script>
