    ...
    with st.form("generator_form"):
        ...
        if generate_btn:
            if not raw_lead.strip():
                st.warning("Please enter lead context — such as LinkedIn bio or job description.")
                st.stop()

            st.session_state.saved_num_openers = num_openers
            messages = [
                {"role": "system", "content": "You are a B2B cold email copywriter. Only return EXACTLY the number of openers requested. Use a numbered list."},
                {"role": "user", "content":
                    f"Write {num_openers} {length.lower()} {style.lower()} cold email openers numbered 1. to {num_openers}.\n"
                    f"Use the following context:\n"
                    f"Lead Info: {raw_lead}\n"
                    f"Company: {company}\n"
