            ...
            with st.form("signup_form"):
                ...
                if signup_btn:
                    if new_pwd != confirm_pwd:
                        ...
                        try:
                            signup_resp = supabase.auth.sign_up({"email": new_email, "password": new_pwd})
                            user = getattr(signup_resp, 'user', None)
                            session = getattr(signup_resp, 'session', None)
                            if session and user:
                                st.session_state["access_token"] = session.access_token
                                st.session_state["refresh_token"] = session.refresh_token
                                st.session_state["authenticated"] = True
                                ...
                                st.info("✅ Check your email to confirm sign-up before logging in.")
                                st.session_state["active_tab"] = "Login"
                                st.rerun()
                        except AuthApiError as e:
        ...
        if st.form_submit_button("✉️ Generate Cold Email"):
            ...
            try:
                with st.spinner("🛠️ Generating your openers..."):
                    ...
                    resp = client.chat.completions.create(model="gpt-4o", messages=messages, max_tokens=num_openers * 80)
    ...
    if "openers" in st.session_state:
        for idx, opener in enumerate(st.session_state["openers"], start=1):
            ...
            if st.session_state.get("view_mode") == "Card View":
                st.markdown(f"<div style='padding:1rem;margin-bottom:1rem;border-radius:12px;"
                            "background-color:rgba(240,240,255,0.1);border:1px solid rgba(200,200,200,0.3);"
                            "box-shadow:0 2px 5px rgba(0,0,0,0.1);'>{opener}</div>", unsafe_allow_html=True)
            else:
                st.markdown(opener)
        ...
        for lead in leads:
            ...
            with st.expander(snippet):
                ...
                if st.button("🗑️ Delete This Lead", key=f"del_{lead['id']}"):
                    try:
                        supabase.table("coldcraft").delete().eq("id", lead["id"
